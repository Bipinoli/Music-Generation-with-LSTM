...
tensorboard = TensorBoard(log_dir="logs/chord_training_shifted_keys")
...
model_name = t+ '-Shifted_%(shifted)s_Lr_%(lr)s_EmDim_%(emdim)s_opt_%(opt)s_bi_%(bi)s_lstmsize_%(lstms)s_trainsize_%(trainsize)s_testsize_%(testsize)s_samples_per_bar%(samples_per_bar)s_dataset_' % fd + source_folder
...
print("--- it took {} seconds to train the chord lstm for -- {} epochs".format(time.time()-start_time,epochs))